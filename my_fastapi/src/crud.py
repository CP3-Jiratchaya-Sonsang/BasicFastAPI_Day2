...
# นำเข้า  BaseModel  จากไลบรารี pydentic สำหรับการสร้าง Schema
from pydantic import BaseModel

# นำเข้า List และ Optional จากไลบรารี ะัยรืเ
...
@app.get("/")
def read_root():
    return {"message": "CRUD Oparation with FastAPI!"}
...
def get_product(product_id: int):
    ...
    # raise คือการโยนข้อผิดพลาด ต่างจาก return ที่เป็นการส่งค่ากลับ
...
def update_product(product_id: int, update_data: ProductUpdate):
    for index, product in enumerate(fake_db): # enumerate เพื่อให้ได้ index ด้วย (index คือ ลำดับที่ของ arry ตัวนั้น)
        if product["id"] == product_id: # เพื่อแตก Dict  เป็น Keyword arguments
